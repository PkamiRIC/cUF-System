...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.level_indicator_labels = {}
        self.level_indicator_leds = {}
        for name, pin in self.level_sensors:
            try:
                plc.pin_mode(pin, plc.INPUT)
            except Exception as e:
                print(f"[WARN] Could not configure level sensor {name} on {pin}: {e}")
        ...
        metrics_panel.setMinimumHeight(240)
        ...
        self.flow_rate_label.setStyleSheet("font-size: 11px;")
        self.total_volume_label.setStyleSheet("font-size: 11px;")
        ...
        self.tmp_plot.setLimits(yMin=0, yMax=200)
        self.tmp_plot.setMinimumHeight(160)
        self.tmp_plot.setMaximumHeight(240)
        ...
        self.idle_time_input = QLineEdit("20.0")
        ...
        for idx, name in enumerate(sequence_names):
            ...
            if handler:
                ...
                btn.clicked.connect(lambda _, n=name: self._on_sequence_clicked(n))
        ...
        for name, _ in self.level_sensors:
            ...
            led.setStyleSheet("background-color: #475569; border-radius: 10px;")

            indicator_layout.addWidget(label)
            indicator_layout.addWidget(led)
            indicator_widget = QWidget()
            indicator_widget.setLayout(indicator_layout)
            level_layout.addWidget(indicator_widget)

            self.level_indicator_labels[name] = label
            self.level_indicator_leds[name] = led
            self._set_level_indicator(name, False)

        controls_row.addWidget(level_container)
    ...
    def _read_level_good_states(self):
        """Return dict of level sensor 'good' states with drain sensors inverted.

        Good means:
        - H2O: HIGH
        - NaOH: HIGH
        - Drain Sample: LOW (inverted)
        - Drain Cleaning: LOW (inverted)
        """
        good = {}
        drain_names = ("Drain Sample", "Drain Cleaning")
        for name, pin in self.level_sensors:
            try:
                raw = bool(plc.digital_read(pin))
            except Exception:
                raw = False
            good[name] = (not raw) if name in drain_names else raw
        return good

    def _check_start_conditions(self):
        """Check if conditions are OK to start a sequence.

        Returns (ok: bool, messages: list[str]) with any required actions.
        """
        states = self._read_level_good_states()
        messages = []
    ...
    def _on_full_sequence_clicked(self):
        ok, msgs = self._check_start_conditions()
        if not ok:
            reasons = "; ".join(msgs)
            self.log_event(f"Cannot start Full Sequence: {reasons}.", context="Full Sequence")
            emit_ui_log(f"Cannot start Full Sequence: {reasons}")
            return
    ...
    def _on_sequence_clicked(self, name: str):
        """Handle individual sequence button presses with pre-check messaging."""
        ok, msgs = self._check_start_conditions()
        if not ok:
            reasons = "; ".join(msgs)
            self.log_event(f"Cannot start {name} sequence: {reasons}.", context=name)
            emit_ui_log(f"Cannot start {name}: {reasons}")
            return
        threading.Thread(target=self.run_sequence, args=(name,), daemon=True).start()
    ...
    def _set_level_indicator(self, name: str, active: bool):
        label = self.level_indicator_labels.get(name)
        led = self.level_indicator_leds.get(name)
        if not label or not led:
            return
        status_text = "OK" if active else ("FULL" if name in ("Drain Sample", "Drain Cleaning") else "LOW")
        label.setText(f"{name}: {status_text}")
        led.setStyleSheet(
            "background-color: #22c55e; border-radius: 8px;"
            if active
            else "background-color: #475569; border-radius: 8px;"
    ...
    def run_sequence(self, name, announce=True):
        ok, msgs = self._check_start_conditions()
        if not ok:
            # Do not start; inform user with specific guidance
            reasons = "; ".join(msgs)
            self.log_event(f"Cannot start {name} sequence: {reasons}.", context=name)
            emit_ui_log(f"Cannot start {name}: {reasons}")
            return
    ...
    def timerEvent(self, event):
        ...
        for name, pin in self.level_sensors:
            try:
                state = bool(plc.digital_read(pin))
            except Exception as e:
                print(f"[WARN] Level sensor read failed for {name} ({pin}): {e}")
                state = False
            # Drain sensors use opposite logic (LOW = OK)
            active = (not state) if name in ("Drain Sample", "Drain Cleaning") else state
            self._set_level_indicator(name, bool(active))
