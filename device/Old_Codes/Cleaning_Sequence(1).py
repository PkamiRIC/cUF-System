...
def run_maf_cleaning_sequence(
    *,
    ...
    """
    MAF CLEANING SEQUENCE  (Steps 1–45)

    This follows the same structure as `run_maf_sequence`:
    - Uses InterruptibleSleeper so it can be aborted safely via `stop_flag`.
    - Each step is executed via `_run_step(label, action, wait_after=...)`.
    - GUI can hook into `before_step(step_label)` to show prompts / dialogs.

    Expected external objects:
      * relays: has .on(idx) / .off(idx)
      * motor_pump: has .set_enabled(bool), .set_direction(bool), .set_speed_checked(bool)
      * pid_controller: has .force_close() and optionally .homing_routine()
      * move_* callbacks move axes as indicated by their names
    """
    ...
    def _home_pid():
        """Home the PID valve using provided callback or controller method."""
        if home_pid_valve:
            home_pid_valve()
            return
    ...
    def _pid_high_speed():
        """Switch peristaltic to HIGH speed (PID already closed)."""
        if motor_pump is None:
            raise RuntimeError("motor_pump object is required")
        motor_pump.set_speed_checked(False)
    ...
    try:
        ...
        _run_step("Step 10: Wait 20 min", wait_after=20 * 60.0) # wait_after=20 * 60.0)
