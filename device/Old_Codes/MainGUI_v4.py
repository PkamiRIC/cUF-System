...
class MainWindow(QWidget):
    def __init__(self):
        ...
        for idx, name in enumerate(sequence_names):
            ...
            if handler:
                ...
                btn.clicked.connect(lambda _, n=name: self._handle_sequence_button(n))
        ...
        def worker():
            ...
            try:
                for step in sequence_order:
                    ...
                    self.run_sequence(step, announce_start=False, log_completion=False)
                    if self.stop_requested:
                        aborted = True
                        break
    ...
    def _handle_sequence_button(self, name: str):
        self.log_event(f"{name} begin", context=name)
        threading.Thread(
    ...
    def _set_level_indicator(self, name: str, active: bool):
        ...
        if not label or not led:
            return
        status_text = "OK" if active else "LOW"
    ...
    def run_sequence(self, name, announce_start=True, log_completion=True):
        self.stop_requested = False
        prev_context = self._log_context
        ...
        try:
            ...
            elif not errored and log_completion:
    ...
    def timerEvent(self, event):
        ...
        for name, pin in self.level_sensors:
            try:
                state = plc.digital_read(pin)
            except Exception as e:
                print(f"[WARN] Level sensor read failed for {name} ({pin}): {e}")
                state = False
            self._set_level_indicator(name, bool(state))
