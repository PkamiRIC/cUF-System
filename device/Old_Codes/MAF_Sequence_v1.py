...
def run_maf_sequence(
    *,
    ...
    def _home_pid():
        """Home the PID valve using a provided callback or controller method."""
        if home_pid_valve:
            home_pid_valve()
            return
    ...
    try:
       ...
       # _run_step("Step 7: Relay 6 OFF", lambda: relays.off(6))
        ...
        _run_step("Step 17: Switch peristaltic to HIGH speed", lambda: motor_pump.set_speed_checked(False),wait_after=30.0)
        ...
        _run_step("Step 23: Syringe move to 1.6 mL", lambda: _syringe_move(1.5, 1.0)) # make value smaller if output sample is too much
        ...
        _run_step("Step 38: Syringe move to 0 mL", lambda: _syringe_move(0, 2.0), wait_after=5.0)
